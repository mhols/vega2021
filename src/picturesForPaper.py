...
#        self.extension = (0.15, 0.3) # limits of bisector area for median calculation vrad_bis
        ...
        self.analyzer = SpectralAnalyser(DATAFILE, normalise=self.normalize)
...
#        self._plt_ls_vr(1000*self.ls_vrad_mean)
    ...
    def mesurementfunktional_vrad(self):
        name = 'mesurementfunktional_vrad'
        eta = self.analyzer.mesurementfunktional_vrad(self.upper, self.lower)
        
        plt.figure()
        plt.plot(self.velocity, eta)
        plt.plot(self.velocity, eta, 'or')
        
        plt.savefig(name + self.format)
    
    def measurementfunctions_vrad_perfect(self):
        
        name = 'measurementfunctions_vrad_perfect'
        x = np.linspace(-0.999,0.999,2048)
        i = 1-np.sqrt(1-x**2)
        di = 2*x / (1-x**2)**(1.5)
        
        U = np.where( (i - 0.1)*(0.3-i) >0 )[0]
        L = np.where( (i - 0.4)*(0.7-i) >0 )[0]

        iu = 1. / (di[U] * np.sum(np.abs(di[U])))
        il = 1. / (di[L] * np.sum(np.abs(di[L])))
