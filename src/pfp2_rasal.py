...
DATAFILE3 = os.path.join(DATADIR, 'titi.dat')  # lines between 0.3 and 1.0 depth, strong
...
DATAFILE13 = os.path.join(DATADIR, 'rasal_2024.dat')
...
nvals[DATAFILE13] = 301
#                    113
#                    268

ranges = {}
ranges[DATAFILE3] = (72, 112)
...
vranges[DATAFILE13] = (-240.0, 240.0)

DATAFILE = DATAFILE13  # DATAFILE8
...
#        for na, nightlist in zip(['s:1', 's:2', 's:3', 's:123456'], [[0], [1], [2], [0,1,2,3,4,5]]):
        ...
        #for na, nightlist in zip(['s:12','s:3','s:123'], [[0,1],[2],[0,1,2]]):
            ...
            plt.imshow(tmp, cmap=plt.cm.gray_r, aspect='auto',interpolation='bicubic', origin='lower',extent=[self.velocity[0]-v0, self.velocity[-1]-v0,0,1],vmin=-0.001,vmax=0.0015)
            #plt.plot(tmp)
            plt.xticks([])
...
#        for na, nightlist in zip(['s:1', 's:2', 's:3', 's:123456'], [[0], [1], [2], [0,1,2,3,4,5]]):
        ...
        #for na, nightlist in zip(['s:12','s:3','s:123'], [[0,1],[2],[0,1,2]]):
            ...
            #plt.imshow(tmp, cmap=plt.cm.gray_r, aspect='auto',interpolation='none', origin='lower',extent=[self.velocity[0]-v0, self.velocity[-1]-v0,0,1])
            plt.imshow(tmp, cmap=plt.cm.gray_r, aspect='auto',interpolation='None', origin='lower',extent=[self.velocity[0]-v0, self.velocity[-1]-v0,0,1],vmin=-0.01,vmax=0.04)
            #plt.plot(tmp)
            plt.xticks([])
            rv = np.array([-20, -10, 0, 10, 20 ])
            st = [ str(i) for i in rv]
