...
For now, the suggested method is to download the whole package and to adjust the PYTHONPATH
to contain the absolute path to the "/nextra/src" source folder. Alternatively you may install
the package into a virtual environment locally by issuing the following 
